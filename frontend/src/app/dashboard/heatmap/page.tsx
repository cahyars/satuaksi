...
import { Map as MapIcon, Radar, AlertTriangle, ShieldCheck, ExternalLink, Activity, Info, Landmark, HelpCircle, ChevronUp, ChevronDown } from 'lucide-react';
import dynamic from 'next/dynamic';

const MapComponent = dynamic(() => import('@/components/EarthquakeMap'), {
  ssr: false,
  loading: () => <div className="absolute inset-0 flex items-center justify-center bg-[#f8fafc] text-cyan-600 font-mono text-xs animate-pulse z-50">MEMUAT PETA SATELIT KEBENCANAAN...</div>
});
...
export default function HeatmapPage() {
  ...
  const fetchBmkgTews = async () => {
    try {
      ...
      toast.error('Gagal memuat data Seismik Nasional');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto lg:h-[calc(100vh-100px)] h-auto flex flex-col">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <span className="text-[10px] font-mono font-bold text-cyan-600 uppercase tracking-widest block font-sans">
            INTEGRASI PENUH RADAR KEBENCANAAN & LIFELINE SAFETY AI
          </span>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 mt-1">Holographic Danger Radar</h1>
          <p className="text-xs text-slate-600 mt-0.5">Pantau peta seismik, telemetry kegempaan nasional, dan analisis tingkat kerawanan wilayah real-time.</p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 self-start md:self-auto">
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-[10px] font-bold font-mono transition-all flex items-center gap-1.5 cursor-pointer ${
              mapMode === 'earthquake'
                ? 'bg-cyan-600 text-white shadow-md shadow-cyan-500/15 font-extrabold'
        ...
        <div className="flex items-center space-x-2.5 bg-white border border-slate-200 px-4 py-2 rounded-2xl shadow-sm">
          <div className={`h-2.5 w-2.5 rounded-full ${(loading || weatherLoading) ? 'bg-yellow-500 animate-pulse' : 'bg-cyan-500 animate-ping'}`}></div>
          <span className="text-[10px] font-mono text-slate-700 font-bold uppercase tracking-wider">
            {(loading || weatherLoading) ? 'Sinkronisasi...' : 'Live Stream Seismik & Gempa'}
          </span>
        </div>
      </div>

      {/* Main Interactive Map & Panel Layout */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-0 h-auto lg:h-full">
        ...
        <div className="lg:col-span-1 glass-panel border-slate-200 rounded-3xl p-5 flex flex-col min-h-0 overflow-hidden relative shadow-sm h-[400px] lg:h-full">
          <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-cyan-500/50 to-blue-500/50" />
          ...
          {loading || weatherLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <Radar className="h-8 w-8 text-cyan-600 animate-spin mb-3" />
              <p className="text-[11px] text-slate-500">Menghubungkan satelit cuaca &amp; radar seismik...</p>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
              ...
              <h4 className="text-xs font-bold text-emerald-600">Indonesia Kondusif</h4>
              <p className="text-[10px] text-slate-500 mt-1">Tidak ada guncangan tektonik baru-baru ini yang terdeteksi.</p>
            </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <div className="flex flex-col ml-auto">
                        <span className="text-[8px] text-cyan-600 font-mono text-right">RISIKO AI</span>
                        <span className="text-xs font-black text-cyan-600 text-right">{eq.dangerScore}%</span>
                      </div>
                    </div>
                  </motion.div>
                );
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1.5">
                      ...
                      <span className="text-[8px] font-mono text-slate-500">{city.province}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <div className="flex flex-col ml-auto">
                        <span className="text-[8px] text-cyan-600 font-mono text-right">RISIKO AI</span>
                        <span className="text-xs font-black text-cyan-600 text-right">{city.dangerScore}%</span>
                      </div>
                    </div>
                  </motion.div>
                );
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 flex justify-between items-center text-[10px]">
            <span className="text-slate-500 font-mono">DIPERBARUI TIAP 3 MENIT</span>
            <button 
              onClick={mapMode === 'earthquake' ? fetchBmkgTews : fetchWeather}
              className="text-cyan-600 hover:text-cyan-700 font-bold flex items-center gap-1 transition-colors"
            >
              <span>Refresh</span>
        ...
        {/* Center/Right: Interactive Map Grid (3/4 width) */}
        <div className="lg:col-span-3 glass-panel border-slate-200 rounded-3xl p-6 relative overflow-hidden flex flex-col min-h-0 shadow-sm h-[550px] lg:h-full">
          {/* Tactical Hologram Overlay Grid background */}
          <div className="absolute inset-0 bg-[#f8fafc] pointer-events-none z-0">
            ...
            {/* Compass Rings */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 rounded-full border border-cyan-500/10 animate-[spin_40s_linear_infinite]" />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full border border-cyan-500/[0.04] animate-[spin_90s_linear_infinite_reverse]" />
          </div>
            ...
            <AnimatePresence mode="wait">
              {mapMode === 'earthquake' && selectedEq && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="glass-panel-glow border-cyan-500/25 bg-white/95 rounded-2xl p-4 md:p-5 shadow-lg relative overflow-hidden flex flex-col"
                >
                  <div className="absolute top-0 left-0 w-2.5 h-full bg-gradient-to-b from-cyan-500 to-blue-600" />
                  
                  {!isCardExpanded ? (
                    /* Collapsed Compact View for Mobile */
                    <div className="flex items-center justify-between gap-3 w-full">
                      ...
                      <button
                        ...
                        className="flex-shrink-0 bg-cyan-600 hover:bg-cyan-700 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold font-mono transition-colors flex items-center gap-1 cursor-pointer"
                      >
                    ...
                    <div className="flex flex-col gap-3">
                      {/* Responsive Minimize/Expand Header */}
                      <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                        <span className="text-[9px] font-mono font-bold text-cyan-600 uppercase tracking-wider">LifeLine AI Danger Assessment</span>
                        <button
                      ...
                      <div className="max-h-[180px] md:max-h-[280px] lg:max-h-none overflow-y-auto pr-1 space-y-2 md:space-y-4 custom-scrollbar flex flex-col md:flex-row gap-2 md:gap-4">
                        ...
                        <div className="flex flex-col items-center justify-center bg-slate-50 border border-slate-200 p-2 md:p-4 rounded-xl text-center md:w-32 flex-shrink-0">
                          <span className="text-[8px] font-mono font-bold text-slate-500 uppercase tracking-widest">Danger Score</span>
                          <motion.span 
                            animate={{ scale: [1, 1.05, 1] }} 
                            transition={{ repeat: Infinity, duration: 2.5 }}
                            className={`text-3xl font-black mt-1 ${
                              selectedEq.dangerScore >= 80 ? 'text-red-600' :
                          ...
                          <span className="text-[8.5px] font-bold text-slate-600 mt-1 uppercase tracking-wider font-mono">
                        ...
                        <div className="flex-1 space-y-2 text-left min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-[8px] font-mono text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded">
                              COORD: {selectedEq.latitude?.toFixed(4)}, {selectedEq.longitude?.toFixed(4)}
                            </span>
                          </div>
                          ...
                          <div className="flex items-start gap-2 bg-cyan-50 border border-cyan-200/60 p-2.5 rounded-xl">
                            <Info className="h-4 w-4 text-cyan-600 flex-shrink-0 mt-0.5" />
                            <div>
                              <h4 className="text-[9px] font-mono font-bold text-cyan-600 uppercase tracking-wider">LifeLine AI Safety Recommendation</h4>
                ...
                >
                  ...
                  {!isCardExpanded ? (
                    ...
                    <div className="flex flex-col gap-3">
                      {/* Responsive Minimize/Expand Header */}
                      <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                        <span className="text-[9px] font-mono font-bold text-cyan-600 uppercase tracking-wider">LifeLine AI Weather Assessment</span>
                        <button
                          onClick={() => setIsCardExpanded(false)}
                          className="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-100 transition-colors lg:hidden"
                        >
                      ...
                      <div className="max-h-[180px] md:max-h-[280px] lg:max-h-none overflow-y-auto pr-1 space-y-2 md:space-y-4 custom-scrollbar flex flex-col md:flex-row gap-2 md:gap-4">
                        {/* Danger Score Gauge */}
                        <div className="flex flex-col items-center justify-center bg-slate-50 border border-slate-200 p-2 md:p-4 rounded-xl text-center md:w-32 flex-shrink-0">
                          <span className="text-[8px] font-mono font-bold text-slate-550 uppercase tracking-widest">Weather Risk</span>
                          <motion.span 
                            animate={{ scale: [1, 1.05, 1] }} 
                            transition={{ repeat: Infinity, duration: 2.5 }}
                            className={`text-3xl font-black mt-1 ${
                              selectedCity.dangerScore >= 80 ? 'text-red-600' :
                        ...
                        <div className="flex-1 space-y-2 text-left min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-[8px] font-mono text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded">
                              COORD: {selectedCity.latitude?.toFixed(4)}, {selectedCity.longitude?.toFixed(4)}
                            </span>
                          </div>

                          <p className="text-[11px] text-slate-650 leading-relaxed font-semibold">
                            Suhu saat ini terpantau <span className="text-slate-800 font-bold">{selectedCity.temperature}°C</span> dengan kelembaban <span className="text-slate-800 font-bold">{selectedCity.humidity}%</span> dan kecepatan angin <span className="text-slate-800 font-bold">{selectedCity.windSpeed} km/h</span>. Cuaca dilaporkan <span className="text-cyan-600 font-bold">{selectedCity.condition}</span>.
                          </p>
                          ...
                          <div className="flex items-start gap-2 bg-cyan-50 border border-cyan-200/60 p-2.5 rounded-xl">
                            <Info className="h-4 w-4 text-cyan-600 flex-shrink-0 mt-0.5" />
                            <div>
                              <h4 className="text-[9px] font-mono font-bold text-cyan-600 uppercase tracking-wider">LifeLine AI WEATHER SAFETY RESPONSE</h4>
                              <p className="text-[10px] text-slate-700 mt-0.5 font-medium leading-relaxed font-mono">
                        ...
                        <div className="md:w-52 bg-slate-50 border border-slate-200 p-3 rounded-xl text-left flex flex-col justify-between">
                          ...
                          <div className="border-t border-slate-200 pt-2 mt-2 flex justify-between text-[10px] font-mono text-cyan-600">
          ...
          <div className="absolute top-6 left-6 z-0 pointer-events-none flex flex-col space-y-1.5 opacity-60">
            <span className="text-[9px] font-mono text-cyan-600 font-bold uppercase tracking-widest flex items-center gap-1.5">
              <Landmark className="h-3 w-3" />
              <span>SISTEM RADAR SEISMIK & METEOROLOGI LIFELINE AI</span>
            </span>
            <span className="text-[8px] font-mono text-slate-500">BOUNDS: INDONESIA ARCHIPELAGO (WIB / WITA / WIT)</span>
          </div>
          ...
          <div className="absolute top-6 right-6 z-0 pointer-events-none flex items-center gap-3">
            ...
            <div className="flex items-center gap-4 bg-white/90 border border-slate-200 px-3.5 py-1.5 rounded-xl backdrop-blur shadow-sm">
              ...
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-cyan-500 shadow-[0_0_6px_rgba(34,211,238,0.3)] animate-pulse" />
